...
```{r, fig.dim=c(6,4) }
design <- ods(Response ~ Month|Patient, 'mixture',
            p_sample=c(1, 0.25, 1),
            data=gbti, quantiles=c(0.1, 0.9), prob_intercept = 0.8)
summary(design)
plot(design)
```
#### Weighted Likelihood
...
design <- ods(Response ~ Month|Patient, 'intercept',
            p_sample=c(1, 0.25, 1),
            data=gbti, weights = "wt", quantiles=c(0.1, 0.9))

data_wt = data.frame(design$model.frame, wt = 1/design$p_sample_i)
est <- WL(Response ~ Month, design, data = data_wt) # weights = 
```


## Demo of ODS Error Input Message
### Error in formula, missing |id

```{r}
try(ods(Response ~ Month,
      method    = "intercept",
      p_sample  = c(1, 0.25, 1),
      data      = gbti,
      quantiles = c(0.1, 0.9)))
...
### Wrong structure of right-hand side
```{r}
try(
  ods(Response ~ Month + Patient,
      method    = "intercept",
      p_sample  = c(1, 0.25, 1),
      data      = gbti,
      quantiles = c(0.1, 0.9))
...
### Error in formula: time is not a single variable

```{r}
try(
  ods(Response ~ I(Month^2 | Patient),
      method    = "intercept",
      p_sample  = c(1, 0.25, 1),
      data      = gbti,
      quantiles = c(0.1, 0.9))
...
### id is not a single variable
```{r}
try(
  ods(Response ~ Month | (Patient + 1),
      method    = "intercept",
      p_sample  = c(1, 0.25, 1),
      data      = gbti,
      quantiles = c(0.1, 0.9))
...
try(
  ods(Response ~ Month | Patient,
      method    = "random",  # not in slope/intercept/bivariate/mean
      p_sample  = c(1, 0.25, 1),
      data      = gbti,
      quantiles = c(0.1, 0.9))
)
...
### Sampling probability length does not match method (bivariate)
```{r}
try(
  ods(Response ~ Month | Patient,
      method    = "bivariate",
      p_sample  = c(1, 0.25, 1),  # should be length of 2
      data      = gbti,
      quantiles = 0.5)
)
```

### Sampling probability length does not match method (intercept)
```{r}
try(
  ods(Response ~ Month | Patient,
      method    = "intercept",
      p_sample  = c(1, 0.25),     # should be length of 3
      data      = gbti,
      quantiles = c(0.1, 0.9))
)
```

### Assigning quantiles and cutpoints together
```{r}
try(
  ods(Response ~ Month | Patient,
      method    = "slope",
...
### Univariate sampling design the length of quantile does not equal to length(p_sample)-1

```{r}
try(
  ods(Response ~ Month | Patient,
      method    = "intercept",                 # or "mean"/"slope"
      p_sample  = c(1, 0.25, 1),
      data      = gbti,
      quantiles = 0.1)                    
)
...
### Bivariate sampling design the length of quantile does not equal to 1
...
### Bivariate sampling design the length of cutpoints does not equal to 4
```{r}
try(
  ods(Response ~ Month | Patient,
      method    = "bivariate",
...
```{r, fig.dim=c(6,4) }
design1 <- bds(formula = Response ~ Genotype + (Month|Patient), 'slope',
              p_sample=c(1, 0.25, 1),
              data=gbti, quantiles=c(0.1, 0.9))
summary(design1)
plot(design1)
...
est <- acml(Response ~ Month*Genotype, design1, data = gbti[gbti$Patient %in% design1$sample_ids,])
summary(est, digits=3)
```
...
```{r, fig.dim=c(6,4) }
design <- bds(Response ~ Month|Patient, 'intercept',
            p_sample=c(1, 0.25, 1),
            data=gbti, quantiles=c(0.1, 0.9))
summary(design)
plot(design)
...
```{r, fig.dim=c(6,4) }
design <- bds(Response ~ Month|Patient, 'bivariate',
            p_sample=c(0.8, 0.2),
            data=gbti, quantiles=c(0.9))
summary(design)
plot(design)
...
## Demo of BDS Error Input Message
### Error in formula, missing |id

```{r}
try(bds(Response ~ Month,
      method    = "intercept",
      p_sample  = c(1, 0.25, 1),
      data      = gbti,
      quantiles = c(0.1, 0.9)))
...
### Error in formula, id format incorrect

```{r}
try(
  bds(Response ~ Genotype + (Month | Patient:Genotype),
      method    = "intercept",
      p_sample  = c(1, 0.25, 1),
      data      = gbti,
      quantiles = c(0.1, 0.9))
...
### Random effect does not match method (no random slope)
```{r}
try(
  bds(Response ~ Genotype + (1 | Patient),
      method    = "slope",
      p_sample  = c(1, 0.25, 1),
      data      = gbti,
      quantiles = c(0.1, 0.9))
...
### Random effect does not match method (no random intercept)

```{r}
try(
  bds(Response ~ Genotype + (0 + Month | Patient),
      method    = "intercept",
      p_sample  = c(1, 0.25, 1),
      data      = gbti,
      quantiles = c(0.1, 0.9))
...
res_int <- data.frame(var = c("beta_0","beta_s","beta_t", "beta_c", "beta_st"),coef_est = round(apply(coef_int, 2, mean),2), bias_int = paste0(round(bias_int,2), "%"),  sd_int = round(sd_int,2), rob_se_est = round(apply(rob_se_int, 2, mean),2), cov_prob = paste0(round(cov_prob,4)*100,"%"), bias_se_int = paste0(round(bias_se_int,2), "%"), bias_rob_se_int = paste0(round(bias_rob_se_int,2), "%"))
kable(res_int, row.names = F, col.names = c("Variable", "Mean of Coef Est", "Coef Est % Bias", "SD of Coef Est", "Robust SE Est","Coverage Prob", "SE Est % Bias", "Robust SE % Bias")) %>%
...
res_slp <- data.frame(var = c("beta_0","beta_s","beta_t", "beta_c", "beta_st"),coef_est = round(apply(coef_slp, 2, mean),2), bias_slp = paste0(round(bias_slp,2), "%"),  sd_slp = round(sd_slp,2), rob_se_est = round(apply(rob_se_slp, 2, mean),2), cov_prob = paste0(round(cov_prob,4)*100,"%"), bias_se_slp = paste0(round(bias_se_slp,2), "%"), bias_rob_se_slp = paste0(round(bias_rob_se_slp,2), "%"))
kable(res_slp, row.names = F, col.names = c("Variable", "Mean of Coef Est", "Coef Est % Bias", "SD of Coef Est", "Robust SE Est","Coverage Prob", "SE Est % Bias", "Robust SE % Bias")) %>%
  kable_styling(full_width = F) %>%
  column_spec(1, bold = T) %>%
  kableExtra::kable_styling(position = "left")
```
