---
title: "ODS Simulation Results"
author: "Shawn Garbett, Lucy Bailu, Jonathan Schildcrout"
date: "2025-06-30"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{ODS Simulation Results}
...
Let's look at a visual summary of the residuals to see that assumptions are met.

```{r plot_acml, eval=FALSE}
plot(est, which=1:2)
```

From this one can see, excellent conformance to the normality assumption.

Further, it's good to check the same assumption for conditional effects.

```{r plot_diagnostics, eval=FALSE}
plot(est, which=3:4)
```
...
- Subjects in the **high** and **low** strata are sampled with probability  
  $\frac{150}{200} = 0.75$

- Subjects in the **central** stratum are sampled with probability  
  $ \frac{100}{1600} = 0.0625 $
